...
public:
    ...
    // rank番目のcの位置 + 1を返す。rankは1-origin
    ...
    // v[begin_pos, end_pos)でrank番目のcのindex + 1．rankは1-origin
    // 未検証
    uint64_t selectRange(const uint64_t c, uint64_t rank, uint64_t begin_pos, uint64_t end_pos) const {
        auto num_c = this->rank(c, begin_pos); // v[0, begin)のcの個数
        auto idx = this->select(c, num_c + 1);
        if (idx >= end_pos) {
            return NOTFOUND;
        }
        return idx;
    ...
    // v[begin_pos, end_pos)で最大値を返す
    uint64_t maxRange(uint64_t begin_pos, uint64_t end_pos) const {
        return quantileRange(begin_pos, end_pos, end_pos - begin_pos - 1);
    }

    // v[begin_pos, end_pos)で最小値を返す
    uint64_t minRange(uint64_t begin_pos, uint64_t end_pos) const {
        return quantileRange(begin_pos, end_pos, 0);
    }

    // v[begin_pos, end_pos)でk番目に小さい値を返す(kは0-origin)
    // つまり小さい順に並べてk番目の値
    uint64_t quantileRange(uint64_t begin_pos, uint64_t end_pos, uint64_t k) const {
        ...
        uint64_t val = 0;
        for (uint64_t depth = 0; depth < this->bit_size; ++depth) {
            const uint64_t num_of_zero_begin = this->bit_arrays.at(depth).rank(0, begin_pos);
            const uint64_t num_of_zero_end = this->bit_arrays.at(depth).rank(0, end_pos);
            ...
            if (bit) {
                k -= num_of_zero;
                begin_pos = this->begin_one.at(depth) + begin_pos - num_of_zero_begin;
                end_pos = this->begin_one.at(depth) + end_pos - num_of_zero_end;
            } else {
    ...
    uint64_t quantileRange_index(uint64_t begin_pos, uint64_t end_pos, uint64_t k) const {
        if ((end_pos > this->size || begin_pos >= end_pos) || (k >= end_pos - begin_pos)) {
            return NOTFOUND;
        }
        ...
        for (uint64_t i = 0; i < this->bit_size; ++i) {
            const uint64_t bit = (val >> (this->bit_size - i - 1)) & 1u;  // 上からi番目のbit
            left = this->bit_arrays.at(i).rank(bit, left);               // cのi番目のbitと同じ数値の数
            if (bit) {
    ...
    uint64_t rangeFreq(uint64_t begin_pos, uint64_t end_pos, uint64_t min_c, uint64_t max_c) const {
    ...
    // v[0, pos)でcより小さい文字の個数
    uint64_t rankLessThan(uint64_t c, uint64_t begin, uint64_t end) const {
        auto t = rankAll(c, begin, end);
        return std::get<1>(t);
    }

    // v[0, pos)でcより大きい文字の個数
    uint64_t rankMoreThan(uint64_t c, uint64_t begin, uint64_t end) const {
        auto t = rankAll(c, begin, end);
        return std::get<2>(t);
    }

    // v[begin, end)で(cと同じ値の個数、cより小さい値の個数、cより大きい値の個数)を求める
    std::tuple<uint64_t, uint64_t, uint64_t> rankAll(const uint64_t c, uint64_t begin, uint64_t end) const {
        assert(end <= size);
    ...
    std::vector<std::pair<uint64_t, uint64_t>> topk(uint64_t s, uint64_t e, uint64_t k) const {
        assert(s < e);
        ...
        std::priority_queue<std::tuple<uint64_t, uint64_t, uint64_t, uint64_t, uint64_t>, std::vector<std::tuple<uint64_t, uint64_t, uint64_t, uint64_t, uint64_t>>, decltype(c)> que(
                c);  // width, left, right, depth, value
    ...
    uint64_t rangeSum(const uint64_t begin, const uint64_t end, const uint64_t x, const uint64_t y) const {
...
//        const uint64_t num = end_pos - begin_pos;
    ...
    std::vector<std::tuple<uint64_t, uint64_t, uint64_t>> intersect(uint64_t _s1, uint64_t _e1, uint64_t _s2, uint64_t _e2) const {
        assert(_s1 < _e1);
        assert(_s2 < _e2);
        ...
        while (not que.empty()) {
            ...
            if (s1_1 != e1_1 and s2_1 != e2_1) {
                que.push(std::make_tuple(s1_1, e1_1, s2_1, e2_1, depth + 1, value | (1u << bit_size - depth - 1u)));
            }
        }
...
private:
    ...
    uint64_t rangeSum(const uint64_t begin, const uint64_t end, const uint64_t depth, const uint64_t c, const uint64_t x, const uint64_t y) const {
        if (begin == end) {
            return 0;
        }
