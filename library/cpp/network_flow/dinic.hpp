...
public:
    struct Edge {
        ...
        Edge(const int to, const T flow, const T cap, const int rev) : to(to), flow(flow), cap(cap), rev(rev) {
            assert(this->cap >= 0);
        }
    };
    ...
    int add_directed_edge(const int from, const int to, const T cap) {
        ...
        assert(cap >= 0);
        this->graph[from].emplace_back(to, 0, cap, static_cast<int>(graph[to].size()));
        this->graph[to].emplace_back(from, cap, cap, static_cast<int>(graph[from].size()) - 1);
        this->edge_id_memo.emplace_back(from, to);
...
private:
    ...
    T dfs(const int u, const int sink, T upper, T delta) {
        ...
        for (int& i = this->current_edge[u]; i < static_cast<int>(this->graph[u].size()); ++i) {
            ...
            const T residual_capacity = e.cap - e.flow;
